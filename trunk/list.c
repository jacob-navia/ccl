...
------------------------------------------------------------------------*/
static ListElement *
NewLink(List * li, const void *data, const char *fname)
{
	ListElement    *result;
...
------------------------------------------------------------------------*/
static int 
Contains(const List * l, const void *data)
{
	size_t          idx;
	if (l == NULL || data == NULL) {
		if (l)
...
------------------------------------------------------------------------*/
static unsigned 
SetFlags(List * l, unsigned newval)
{
	unsigned        result;
...
static List    *
SetAllocator(List * l, ContainerMemoryManager * allocator)
{
	if (l == NULL) {
...
SetCompareFunction(List * l, CompareFunction fn)
{
...
/*------------------------------------------------------------------------
 ...
 ------------------------------------------------------------------------*/
static int 
CopyElement(const List * l, size_t position, void *outBuffer)
{
	ListElement    *rvp;
...
static int 
ReplaceAt(List * l, size_t position, const void *data)
{
	ListElement    *rvp;
...
------------------------------------------------------------------------*/
static int 
Equal(const List * l1, const List * l2)
{
...
static int 
EraseRange(List * l, size_t start, size_t end)
{
...
static int 
AddRange(List * AL, size_t n, const void *data)
{
 ...
 */
static int 
IndexOf_nd(const List * l, const void *ElementToFind, void *ExtraArgs, size_t * result)
{
	ListElement    *rvp;
	int             r;
	size_t          i = 0;
...
static int 
IndexOf(const List * l, const void *ElementToFind, void *ExtraArgs, size_t * result)
{
...
static int 
lcompar(const void *elem1, const void *elem2, CompareInfo * ExtraArgs)
{
...
SetErrorFunction(List * l, ErrorFunction fn)
{
	ErrorFunction   old;
	if (l == NULL) {
...
static int 
ReplaceWithIterator(Iterator * it, void *data, int direction)
{
	struct ListIterator *li = (struct ListIterator *) it;
	int             result;
	size_t          pos;
...
static int 
DefaultSaveFunction(const void *element, void *arg, FILE * Outfile)
{
...
static int 
Save(const List * L, FILE * stream, SaveFunction saveFn, void *arg)
{
	size_t          i;
	size_t          elemsiz;
	ListElement    *rvp;
...
static int 
DefaultLoadFunction(void *element, void *arg, FILE * Infile)
{
	size_t          len = *(size_t *) arg;
...
static const ContainerMemoryManager *
GetAllocator(const List * l)
{
	if (l == NULL)
		return NULL;
	return l->Allocator;
